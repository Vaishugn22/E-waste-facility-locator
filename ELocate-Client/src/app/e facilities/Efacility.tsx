...
const FacilityMap: React.FC = () => {
  ...
  const [attemptedManualLocation, setAttemptedManualLocation] = useState(false);
  ...
  // Attempt to get the user's current location on mount
  useEffect(() => {
    (async () => {
      ...
      if (coordinates) {
        ...
        // If no coordinates, user likely denied or no HTTPS environment.
  ...
  const handleAllowLocationClick = () => {
    setAttemptedManualLocation(true);
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // If permission was denied before, no prompt will appear.
          // Must reset permissions in browser or run over HTTPS.
        },
      ...
      .then((data: Facility[]) => {
        ...
        geocoder.on("result", (event) => {
          ...
          if (geometry && geometry.coordinates) {
            setClientLocation(geometry.coordinates as [number, number]);
          }
        });
        ...
        <div className="px-4 w-full md:h-[70vh] py-16 md:pt-64 md:py-24 md:container text-center">
          <div className="flex flex-col items-center justify-center px-10">
            ...
            <div className="text-black text-center text-xl md:text-3xl mt-4">
              Enable location services or click "Allow Location" below.
            </div>
            <button
            ...
            {attemptedManualLocation && (
              <p className="text-red-500 mt-4">
                If still no prompt, ensure you're using HTTPS and haven't previously denied permissions.
              </p>
            )}
          </div>
        </div>
